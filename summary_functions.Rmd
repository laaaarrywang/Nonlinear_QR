...
```{r}
function (object, ntrace = 1000, burn.perc = 0.5, plot.dev = TRUE, 
    more.details = FALSE, ...)
  # ntrace: number of draws to be included in trace plots
{
    ...
    if (more.details) {
        ...
        lam.priorsamp <- lamFn(sample(object$prox, ntrace, replace = TRUE, 
            prob = prior.grid))
        ...
        for (i in 1:(p + 1)) {
            ...
            lines(thin * ss, b * (i - 1) + lam.samp[i, ], lwd = 1, 
                col = 4)
            if (i%%2) 
                axis(2, at = b * (i - 1) + lamFn(object$prox[c(1, 
        ...
        theta <- as.mcmc(t(matrix(object$parsamp, ncol = nsamp)[, 
            ss[post.burn]]))
        gg <- geweke.diag(theta, 0.1, 0.5)
        zvals <- gg$z
