...
fit.qrj <- qrjoint(BetaPlasma ~ Age + Sex + SmokStat + Quetelet + VitUse + Calories + 
        Fat + Fiber + Alcohol + Cholesterol + BetaDiet, plasma, nsamp = 500, thin = 20)
summary(fit.qrj, more = TRUE)
```

```{r}
# calculate interior point by averaging extreme points
p <- 9
n <- 200
u <- runif(n)
require(splines)
x <- bs(u, df = p) # generate constant term omitted cubic-spline functions with 9-3=6 knots.
x
...
#{
    ...
    mf = match.call(qrjoint, call("qrjoint", BetaPlasma ~ Age + Sex + SmokStat + Quetelet + VitUse + Calories +  Fat + Fiber + Alcohol + Cholesterol + BetaDiet, plasma, nsamp = 1000, thin = 10), expand.dots = F)
...
```{r}
results = as_tibble(matrix(oo$parsamp,500,113,TRUE))
results = matrix(oo$parsamp,500,113,TRUE)
summary(oo,more.details = TRUE)

results2 = tibble(matrix(fit.qrj$parsamp,500,100,TRUE))
results2 = matrix(fit.qrj$parsamp,500,100,TRUE)
