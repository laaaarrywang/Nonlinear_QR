qrjointL <- function (formula, data, nsamp = 1000, thin = 10, cens = NULL, 
                      ...
                      fix.nu = FALSE, fbase = c("t", "logistic", "unif"), verbose = TRUE, sparse = TRUE,seed = 1026) 
{
  ...
  if (fbase.choice == 1) { # t-distribution is used as the base
    ...
    q0 <- function(u, nu = Inf) return(1/dlogis(qlogis(qrjoint:::unitFn(u))))
    Q0 <- function(u, nu = Inf) return(qlogis(qrjoint:::unitFn(u)))
  ...
  lamsq.grid <- qrjoint:::lamFn(prox.grid)^2
  ...
  if (par[1] == "prior") {
    ...
    sigma <- 1
    par[(nknots + 1) * (p + 1) + 1] <- qrjoint:::sigFn.inv(sigma, a.sig)
    for (i in 1:(p + 1)) {
      ...
      if (substr(blocking, 1, 3) == "std") { # "std0" to "std7"
        ...
        if (as.numeric(substr(blocking, 4, 5)) > 4) { # “std5-std7”
          ...
          if (illcond) {
            ...
            slist[[p + 2]] <- summary(suppressWarnings(quantreg::rq(quantreg::dither(y) ~ 
                                                                      x, tau = 0.5, weights = wt)), se = "boot", 
                                      cov = TRUE)$cov
          }
          slist[[p + 3]] <- matrix(c(1, 0, 0, 0.1), 2, 
                                   2)[sig.nu, sig.nu]
          blocks.S[[p + 4]] <- as.matrix(Matrix::bdiag(slist)) # all parameters
          if (blocking == "std6") {
            for (i in 1:p) blocks.S[[p + 4 + i]] <- as.matrix(Matrix::bdiag(slist[c(1, 
                                                                            i + 1)]))
          }
        }
      }
